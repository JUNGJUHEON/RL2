...
def setup_hardware(use_gpu=True, gpu_memory_limit=None):
    ...
    def _status(mode, physical_gpus=None, logical_gpus=None, note=None):
        physical_gpus = physical_gpus or []
        logical_gpus = logical_gpus or []
        status = {
            "requested_mode": mode,
            ...
            "using_cuda": bool(logical_gpus or physical_gpus),
        }
        if note:
            status["note"] = note
        return status
    ...
    def _print_status(status):
        physical = ", ".join(status["physical_gpus"]) or "none"
        logical = ", ".join(status["logical_gpus"]) or "none"
        cuda_state = "ON" if status["using_cuda"] else "OFF"
        print_info(
            "[Hardware] "
            f"CUDA={cuda_state} | requested={status['requested_mode']} | "
            f"TF built with CUDA={status['tensorflow_built_with_cuda']} | "
            f"physical GPUs={physical} | logical GPUs={logical}"
        )
    ...
    if use_gpu:
        ...
        try:
            ...
            print_warning(f"Could not hide GPU because TensorFlow is already initialized: {exc}")
        status = _status("CPU", [], [])
    ...
    if len(gpus) == 0:
        print_warning("There are no GPUs visible to TensorFlow! Continuing with CPU only.")
        status = _status("GPU", [], [])
        _print_status(status)
        return status
    ...
    try:
        if gpu_memory_limit is not None:
            ...
            print_success(f"Using GPU with memory limit {gpu_memory_limit} MB: {gpus[0].name}")
        else:
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
            print_success("Using GPU(s): " + ", ".join(gpu.name for gpu in gpus))
    except RuntimeError as exc:
        print_warning(f"GPU config could not be changed because TensorFlow is already initialized: {exc}")
    logical_gpus = tf.config.list_logical_devices('GPU')
    status = _status("GPU", gpus, logical_gpus)
    _print_status(status)
    return status
