...
class Statistics:
    ...
    def print_stats(self, par_step, total_par_steps, print_stats_period, num_par_envs,
        ...
        # 진행률 & ETA 계산
        pct = par_step / total_par_steps * 100
        bar_len = 30
        filled = int(bar_len * par_step // total_par_steps)
        try:
            "█░".encode(os.device_encoding(1) or "utf-8")
            bar = "█" * filled + "░" * (bar_len - filled)
        except (UnicodeEncodeError, LookupError):
            bar = "#" * filled + "-" * (bar_len - filled)
        if total_time > 0 and par_step > 0:
            eta_sec = total_time / par_step * (total_par_steps - par_step)
            eta_str = sec2hhmmss(eta_sec)
        else:
            eta_str = "--:--:--"

        stats_text = "\n" + bold("진행률: [%s] %.1f%%" % (bar, pct)) + \
                     bold("  (스텝 %d / %d  |  ETA %s)" % (par_step, total_par_steps, eta_str)) + \
    ...
    def plot_scores(self, out_path):
        ...
        if len(scores) > 0:
            self.plot_real_time(y=scores, y_records=self.get_score_records(), title="Score history",
                                y_label="Score", out_path=out_path + "scores.png", show=False)
    ...
    def plot_score_dist(self, out_path):
        ...
        if len(scores) == 1000:
            ...
            fig, ax = plt.subplots()
            ax.hist(scores_grouped, bins=bins, histtype="barstacked", rwidth=0.7, color=colors)
            cmap2 = plt.cm.get_cmap('Blues_r', n_groups)
            cbar = fig.colorbar(plt.cm.ScalarMappable(norm=None, cmap=cmap2), ax=ax, ticks=[0, 1])
