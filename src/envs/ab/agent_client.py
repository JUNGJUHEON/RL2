...
class AgentClient:
    ...
    def __init__(
        ...
        # Keep the training progress bar readable. The client calls screenshot
        # and gamestate endpoints every step, so INFO logging is very noisy.
        # Set AIBIRDS_AGENT_CLIENT_LOG_LEVEL=INFO only when debugging the wire protocol.
        client_log_level = os.environ.get("AIBIRDS_AGENT_CLIENT_LOG_LEVEL", "QUIET").upper()
        if client_log_level in {"QUIET", "OFF", "NONE", "0", "FALSE"}:
            self._logger.disabled = True
        else:
            self._logger.disabled = False
            self._logger.setLevel(getattr(logging, client_log_level, logging.WARNING))
    ...
    def connect_to_server(self):
        try:
            ...
            self.server_socket.settimeout(30)  # 30s: 블록 방지
            self._logger.info(
    ...
    def get_game_state(self):
        ...
        value = self._read_from_buff("B")[0]
        try:
            state = GameState(value)
        except ValueError:
            self._logger.warning("Unknown game state value: %d, treating as UNKNOWN", value)
    ...
    def ready_for_new_set(self):
        """Signal readiness for a new training/test set (required after configure() on Linux).

        The server responds with a 19-byte level-set descriptor:
          bytes  0-3 : trial_start (int)
          bytes  4-7 : trial_end   (int)
          bytes  8-11: novelty_count (int, 0 in training mode)
          bytes 12-15: attempt_count (int)
          byte  16   : 0=training mode, 1=competition mode
          byte  17   : 0=sequential, 1=random
          byte  18   : novelty flag
        """
        self._logger.info("Sending readyForNewSet request")
        self._send_command(RequestCodes.ReadyForNewSet)
        raw = self._read_raw_from_buff(19)
        trial_start   = int.from_bytes(raw[0:4],  byteorder='big', signed=True)
        trial_end     = int.from_bytes(raw[4:8],  byteorder='big', signed=True)
        novelty_count = int.from_bytes(raw[8:12], byteorder='big', signed=True)
        attempt_count = int.from_bytes(raw[12:16], byteorder='big', signed=True)
        is_training   = raw[16] == 0
        is_sequential = raw[17] == 0
        self._logger.info(
            "readyForNewSet: trial=%d~%d, novelty_count=%d, attempts=%d, training=%s",
            trial_start, trial_end, novelty_count, attempt_count, is_training
        )
        return (trial_start, trial_end, novelty_count, attempt_count)

    def report_novelty_likelihood(self, likelihood=0.0):
        """Report novelty likelihood (required when state is REQUESTNOVELTYLIKELIHOOD).

        Wire format (after command byte):
          4B float : novelty_likelihood
          4B float : secondary_metric (always 0.0)
          4B int   : N = novelty_type array length (0 = none)
          N*4B int[]: novelty type IDs
          4B int   : type_id (0 for training)
          4B int   : L = description string length (0 = empty)
          L bytes  : UTF-8 description string

        Java responds with 1 byte: OK(1) or ERR(0).
        """
        self._logger.info("Sending reportNoveltyLikelihood request (likelihood=%.2f)", likelihood)
        self._send_command(
            RequestCodes.ReportNoveltyLikelihood,
            "ffIII",
            likelihood,  # novelty_likelihood float
            0.0,         # secondary_metric float
            0,           # N = 0 (empty int array)
            0,           # type_id = 0
            0,           # L = 0 (empty description string)
        )
        response = self._read_from_buff("B")[0]
        self._logger.info("reportNoveltyLikelihood response: %d", response)
        return response
